...
//            3.opencsv
            ...
            String fileName = "C:/Users/chirag/Documents/Ushma/webProjectNew/articles/NYTDataset-updated.csv";//args[0];
            ArrayList<String> Query_input = new ArrayList<>();
            ArrayList<String> Query_resolved = new ArrayList<>();
            String useDelimiter = "[ ]|,|\"|\\||-|[']|[.]|[(]|[)]|[/]|[”]|[“]|[’]|[‘]|[ ]";
            Scanner scone = new Scanner((System.in));
            while(true) {
                System.out.println("Enter 1 to enter the query or 0 to exit:");
                Integer num = scone.nextInt();
                if(num == 0){
                    break;
                }
                else {
                    ...
                    String[] temp = qu.replaceAll("\"", "").split(useDelimiter);
                    //System.out.println(qu);
                    Query_input.clear();
                    for (int i = 0; i < temp.length; i++) {
                        Query_input.add(temp[i]);
                        //System.out.println(Query_input);
                    }

                    int bol = 0;
                    if ((phrase != 1) && ((Query_input.get(1).equals("AND")) || (Query_input.get(1).equals("OR")) || ((Query_input.get(1).equals("NOT"))))) {
                        bol = 1;
                    }
                    //Query_resolved.clear();
                    Query_resolved = Query_input;
...
//-----------------------------------------------------------------------------------------------
    ...
    public static void BolRetnew(ArrayList<String> query, String fileName)
    {
        ...
        Map<Integer, String> map_author = new HashMap<>();
        ...
        for(Integer q:queryzero_present_art){
            ...
            if(query.get(1).equals("NOT")){
                if(!data.contains(Query_stem.get(1))){
                    System.out.println("Title:"+map_head.get(q));
                    System.out.println("Authors:"+map_author.get(q));
                    System.out.println("URL:"+map_url.get(q));
                    System.out.println("");
                }
    ...
    public static void Phrase(ArrayList<String> query, String fileName)
    {
        ...
        for(Integer q:queryzero_present_art){
            ...
            if(data.toLowerCase().contains(query_together)){
                ...
                System.out.println("Sentences containing query:");
    ...
    public static void Query(ArrayList<String> query, String fileName)
    {
        ...
        sortedmap_pqd.entrySet().forEach(entry->{
            System.out.println(/*entry.getKey()+ ","+ + entry.getValue() +"," + */"Title:"+map_head.get(entry.getKey()));
            System.out.println(/*entry.getKey()+ ","+ + entry.getValue() +"," + */"Author:"+map_author.get(entry.getKey()));
            System.out.println(/*entry.getKey()+ ","+ + entry.getValue() +"," + */"URL:"+map_url.get(entry.getKey()));
            System.out.println(/*entry.getKey()+ ","+ + entry.getValue() +"," + */"Score:"+sortedmap_pqd.get(entry.getKey()));
